    ...
    <MobileShell showTabs>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Stat
            value={
              metrics.tasks.total ? `${metrics.tasks.completionRate}%` : "--"
            }
            label="任务完成率"
            icon={<Briefcase />}
          />
        ...
        >
          <ProgressBar
            value={metrics.tasks.completed}
            max={Math.max(1, metrics.tasks.total)}
          />
          <Link
            to="/tasks"
          ...
          >
            <span>
              {metrics.tasks.total ? "查看执行中心" : "前往生成执行任务"}
            </span>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                  {metrics.bestProject.valueForResume ||
                    "该项目用于证明目标岗位需要的核心能力。"}
                </p>
              </div>
